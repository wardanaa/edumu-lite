...
@section('title', __('Edumu-Lite') . ' | ' . __('Edit Ebook'))
...
@section('content')
    {{ html()->modelForm($content, 'PATCH', route('admin.ebook.editpostebook', $content->id))->class('form-horizontal')->attribute('enctype', 'multipart/form-data')->open() }}
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        @lang('Ebook')
                        <small class="text-muted">@lang('Edit Ebook')</small>
                    </h4>
                </div><!--col-->
            </div><!--row-->
            ...
            <div class="row mt-4 mb-4">
                <div class="col">
                        ...
                        <div class="col-md-10">
                            <select class="form-control custom-select"  name="content_category_id">
                                ...
                                @foreach($category as $con)
                                    <option value="{{ $con->id }}"{{ $content->content_category_id == $con->id ? 'selected' : ''}}>{{ $con->name}}</option>
                                @endforeach
                            </select>
                        </div><!--col-->
                    </div><!--form-group-->
                    ...
                    <div class="row">
                        {{ html()->label(__('File Upload'))->class('col-md-2 form-control-label')->for('file') }}
                        <div class="col-10 col-sm-6 col-lg-8">
                            <a href="{{$content->file_location}}">{{$content->file_location}}</a>
                        </div>
                    </div>
